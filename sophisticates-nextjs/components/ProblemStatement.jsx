                    ...
                    {problems.map((item, i) => (
                        ...
                        >
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', paddingTop: '4px' }}>
                                <span style={{ fontFamily: 'var(--font-accent)', fontSize: '0.65rem', color: 'var(--text-dim)', letterSpacing: '0.2em' }}>
                                    {item.index}
                                </span>
                                {!isMobile && (
                                    <span style={{ fontFamily: 'var(--font-body)', fontSize: '0.6rem', color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: '0.2em', lineHeight: 1.4 }}>
                                        {item.domain}
                                    </span>
                                )}
                            </div>
                            ...
                            <div>
                                {isMobile && (
                                    <span style={{ display: 'block', fontFamily: 'var(--font-body)', fontSize: '0.6rem', color: 'var(--text-dim)', textTransform: 'uppercase', letterSpacing: '0.2em', marginBottom: '12px' }}>
                                        {item.domain}
                                    </span>
                                )}
                                <h3 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(1.5rem, 2.5vw, 2.4rem)', fontWeight: 400, color: 'var(--text-main)', letterSpacing: '-0.03em', lineHeight: 1.05, margin: 0 }}>
                                    {item.problem}
                                </h3>
                            </div>

                            <p style={{ fontFamily: 'var(--font-body)', fontSize: 'clamp(0.9rem, 1.1vw, 1.05rem)', color: 'var(--text-muted)', lineHeight: '1.7', fontWeight: 300, margin: 0, paddingTop: isMobile ? '0' : '6px' }}>
                                {item.body}
                            </p>
                        </motion.div>
                    ))}
